...
class TextureDict:
    ...
    def _convert_xbox360_inplace(self, tex, new_gpu, rgba, w, h):
        ...
        #    the format-dependent tiling register dword_11). dword_11 is keyed to
        #    the texture FORMAT, not its size -- every DXT1/3/5 texture uses
        #    0x00000D10, every DXT5A uses 0x000015B6, 8_8_8_8 uses 0x00000C14
        #    (verified against a real stock hud.xtd). If we change the format but
        #    leave dword_11 on the old DXT5A value, the GPU keeps reading the
        #    texture in single-channel mode -> it renders monochrome no matter
        #    what colour is painted. This was the radio-wheel "no colour" bug.
        DWORD11_FOR_FMT = {
            18: 0x00000D10,  # DXT1
        ...
        for (t, fmt, tiled, dwSize), off in zip(per_tex, offsets):
            ...
            # dword_11 (d3d+0x28): rewrite to the value matching the NEW format.
