...
class TextureDict:
    ...
    def save(self, out_path):
        ...
        if any_converted and not gpu_grew:
            # PC path (or any platform where we couldn't do a surgical edit):
            # rebuild the whole dictionary from the live textures.
            if self.platform == "PC":
        ...
        if gpu_grew:
            # The surgical conversion appended data to the GPU block, so the
            # decompressed buffer (and thus the graphics segment) is larger.
            # Recompute the graphics flag so GetValueRSC7 reports the new size;
            # the system flag is unchanged (we only touched the GPU block).
            gpu_len = len(body) - self.cpu_size
            gfx_flag, gfx_pages = _rsc7_flag_for_size(gpu_len, 0x2000, 13)
            # pad the GPU block out to the page count the flag describes
            need = self.cpu_size + gfx_pages
            if len(body) < need:
                body = body + b'\x00' * (need - len(body))
            # header layout: '7CSR'(4) ver(4) sysFlag(4, LE of EndianChanged)
            #                gfxFlag(4, LE of EndianChanged)
            struct.pack_into('<I', header, 12, EndianChangeDWORD(gfx_flag))
            self.gpu_size = gpu_len
    ...
    def convert_texture_format(self, tex, new_fmt):
        ...
        if new_fmt == 'DXT5' and (tex.fmt_name or '').upper() in ('DXT5A','ATI1','BC4'):
            # Expand single channel (DXT5A red) -> opaque RGB grayscale.
            ba = bytearray(rgba)
            for i in range(0, len(ba), 4):
                r = ba[i]; ba[i]=r; ba[i+1]=r; ba[i+2]=r; ba[i+3]=255
            rgba = bytes(ba)
    ...
    def _convert_xbox360_inplace(self, tex, new_gpu, rgba, w, h):
        """Append re-encoded DXT data to the GPU block and re-point the texture's
        fetch constant + format bits. Grows _dec; save() recomputes gfx flag."""
        self._ensure_mutable()
        # Encode + tile the new data for the target format.
        linear = encode_for_format(new_gpu, rgba, w, h)
        tiled  = retile_and_swap(linear, new_gpu, w, h)

        # Append to the END of the decompressed buffer (which ends with the GPU
        # block). Page-align the new data start to 0x1000, like real files.
        new_gpu_off_abs = _align(len(self._dec), 0x1000)
        if new_gpu_off_abs > len(self._dec):
            self._dec.extend(b'\x00' * (new_gpu_off_abs - len(self._dec)))
        self._dec.extend(tiled)
        # offset relative to the start of the GPU block (cpu_size is the split)
        new_gpu_off_rel = new_gpu_off_abs - self.cpu_size

        # Patch the texture's fetch constant in the SYSTEM block.
        # We need the d3d struct offset for this texture; re-derive it from the
        # struct so we don't depend on parse-time state.
        d3doff = self._xbox_d3d_offset(tex)
        if d3doff is None:
            raise RuntimeError("Could not locate the texture's fetch constant "
                               "for in-place conversion.")
        # dword at d3doff+0x20 holds: base_address(top 20 bits) | endian | format
        old = struct.unpack_from('>I', self._dec, d3doff + 0x20)[0]
        fmt_idx = XBOX_FMT_CODE.get(new_gpu.replace('GPUTEXTUREFORMAT_','')
                                    .replace('DXT4_5','DXT5').replace('DXT2_3','DXT3'), 20)
        # rebuild: keep the upper address bits + endian (bit6), set new format.
        gpu_paged = _ps3_vaddr(0x6, new_gpu_off_rel)
        endian_bit = old & (1 << 6)
        dword9 = (gpu_paged & 0xFFFFF000) | endian_bit | (fmt_idx & 0x3F)
        struct.pack_into('>I', self._dec, d3doff + 0x20, dword9 & 0xFFFFFFFF)

        # We emit only the base mip, so set MaxMipLevel = 0 (single mip). The
        # mip count lives in specific bits of dword_12 (d3d+0x2C); clearing the
        # WHOLE dword would wipe other GPU fields and crash the game. The reader
        # decodes MaxMip from bits 0xC0000000 and 0x00030000 (after endian swap),
        # which in the stored big-endian dword are these byte positions:
        old12 = struct.unpack_from('>I', self._dec, d3doff + 0x2C)[0]
        # In the little-endian value the reader sees, mip bits are 0xC0030000.
        # Map that mask through the same endian swap to the stored BE dword.
        mip_mask_le = 0xC0030000
        mip_mask_be = EndianChangeDWORD(mip_mask_le)
        new12 = old12 & (~mip_mask_be & 0xFFFFFFFF)   # zero only the mip bits
        struct.pack_into('>I', self._dec, d3doff + 0x2C, new12 & 0xFFFFFFFF)
        # mirror the single-mip change in the struct's level field (base+0x3C).
        bb = self._xbox_struct_base(tex)
        if bb is not None:
            struct.pack_into('>I', self._dec, bb + 0x3C, EndianChangeDWORD(1))

        # Update the live TexEntry so preview + later saves are consistent.
        tex.tex_offset = new_gpu_off_rel
        tex.texture_type = fmt_idx
        tex.mips = 1
        tex._converted = True
        # Mark that the GPU block grew, so save() recomputes the graphics flag.
        self._gpu_grew = True
...
class App(tk.Tk):
    ...
    def _convert_format_sel(self):
        ...
        if cur in ("DXT5A","ATI1","BC4"):
            tk.Label(win,
                text="DXT5A holds a single channel. Converting to DXT5 expands\n"
                     "it to a colour-capable image (grayscale to start). Replace\n"
                     "it afterwards with a colour image to use full colour.",
        ...
        for f in ("DXT5","DXT3","DXT1"):
            tk.Radiobutton(rowf, text=f, variable=choice, value=f,
