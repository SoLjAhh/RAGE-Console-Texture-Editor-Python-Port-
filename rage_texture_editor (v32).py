...
class TextureDict:
    ...
    def _replace_texture_OLD(self, tex, image_path):
        if tex.platform == "PC":
            return self._replace_pc(tex, image_path)
        elif tex.platform == "Xbox 360":
            return self._replace_xbox360(tex, image_path)
    ...
    def save(self, out_path):
        ...
        if any_converted:
            if self.platform == "Xbox 360":
                return self.convert_to_xbox360(out_path, compress=True)
    ...
    def convert_texture_format(self, tex, new_fmt):
        ...
        full colour. This is NON-DESTRUCTIVE to the shared GPU buffer: the
        current pixels are decoded to RGBA and stashed on the TexEntry, and the
        format field is updated. The preview reflects the change immediately,
        and the next Save As rebuilds the dictionary with the new format and
        correctly-sized data.
        ...
        which the user can then Replace with a full-colour image.
        """
        ...
        if new_fmt == 'DXT5':
            # Expand a single-channel (DXT5A/BC4 red) texture into an opaque RGB
            # grayscale so it reads as a normal colour image after conversion.
            if (tex.fmt_name or '').upper() in ('DXT5A', 'ATI1', 'BC4'):
                ba = bytearray(rgba)
                for i in range(0, len(ba), 4):
                    r = ba[i]
                    ba[i] = r; ba[i+1] = r; ba[i+2] = r; ba[i+3] = 255
                rgba = bytes(ba)

        tex._rgba_override = (rgba, w, h)
        tex._converted = True
        tex.fmt_name = new_fmt
        tex.gpu_fmt  = gpu_map[new_fmt]
        return True
