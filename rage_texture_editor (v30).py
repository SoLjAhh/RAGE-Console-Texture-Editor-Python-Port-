...
class RPFBrowserWindow(tk.Toplevel):
    ...
    def _build_main_area(self):
        ...
        self._file_tree.bind("<Double-1>", lambda _: self._open_in_editor())
    ...
    def _open_archive(self):
        ...
        if key_path:
            ...
            # Try unencrypted / OPEN; we pass a dummy key
            self.key = b"\x00" * 32
            self._setstatus("No key supplied — trying unencrypted (OPEN) mode…")

        self._setstatus(f"Opening {Path(arc_path).name}…")
        try:
            self.arc = RPFArchive.open(arc_path, self.key,
                                       lzx=self.app.lzx, platform=plat)
        except RPFError as e:
            messagebox.showerror("RPF Error", str(e), parent=self)
            self._setstatus("Failed to open archive."); return
        except Exception as e:
            messagebox.showerror("Error", str(e), parent=self)
            self._setstatus("Failed."); return
    ...
    def _populate_trees(self):
        ...
        # --- flat file list (all files, filtered per selection later) ---
        self._all_file_entries = list(self.arc.iter_files())
        self._show_files(self._all_file_entries)
    ...
    def _show_files(self, entries):
        ...
        for e in entries:
            ...
            kind = "RES" if e.is_resource else ("CMP" if e.is_compressed else "RAW")
            ...
            tag  = "tex" if ext in self.TEXTURE_EXTS else ("res" if e.is_resource else "norm")
    ...
    def _on_dir_select(self, _=None):
        ...
        # show files that are direct children of this directory
        files = [c for c in entry.children if isinstance(c, RPFFileEntry)]
        self._show_files(files)
        self._setstatus(f"{entry.path or '(root)'}  —  {len(files)} file(s)")
        self._apply_filter()
    ...
    def _apply_filter(self, *_):
        ...
        if not filt:
            self._show_files(self._all_file_entries)
            return
        filtered = [e for e in self._all_file_entries
