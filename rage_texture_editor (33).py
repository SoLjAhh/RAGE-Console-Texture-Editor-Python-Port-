...
class TextureDict:
    ...
    def _convert_xbox360_inplace(self, tex, new_gpu, rgba, w, h):
        ...
        # We emit only the base mip, so force MaxMipLevel = 0 (single mip) in
        # dword_12 (d3d+0x2C). Leaving a higher mip count would make the game
        # read mip data that no longer exists -> crash.
        struct.pack_into('>I', self._dec, d3doff + 0x2C, 0)
