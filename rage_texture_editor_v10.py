...
class App(tk.Tk):
    ...
    def _refresh_pending_view(self):
        ...
        for i, t in enumerate(self._pending):
            self._tree.insert("","end",iid=str(i),
                values=(t['name'], f"{t['width']}x{t['height']}", t['fmt'], 1, "PC*"))
    ...
    def _ask_format(self):
        ...
        # ---- Compression format ----
        tk.Label(win, text="Compression format:", bg=BG, fg=FG,
                 font=("Segoe UI", 10), padx=16, pady=(12, 4)).pack(anchor="w")
        fmtvar = tk.StringVar(value="DXT5")
        fmt_frame = tk.Frame(win, bg=BG)
        fmt_frame.pack(fill="x", padx=24, pady=(0, 4))
        for f, desc in [("DXT5", "DXT5  (color + alpha)"),
                         ("DXT1", "DXT1  (opaque / 1-bit alpha)"),
                         ("DXT3", "DXT3  (sharp / punch-through alpha)")]:
            tk.Radiobutton(fmt_frame, text=desc, variable=fmtvar, value=f,
        ...
        # ---- Mip levels ----
        tk.Label(win, text="Mip levels:", bg=BG, fg=FG,
                 font=("Segoe UI", 10), padx=16, pady=(10, 4)).pack(anchor="w")
        mipvar = tk.StringVar(value="1")
        mip_frame = tk.Frame(win, bg=BG)
        mip_frame.pack(fill="x", padx=24, pady=(0, 4))
        for label, val in [("1  (no mips)", "1"),
                            ("Full chain  (auto)", "auto"),
                            ("Custom:", "custom")]:
            tk.Radiobutton(mip_frame, text=label, variable=mipvar, value=val,
                           bg=BG, fg=FG, selectcolor=PANEL,
                           activebackground=BG, activeforeground=ACCENT,
                           font=("Segoe UI", 9), anchor="w").pack(fill="x", pady=1)

        custom_row = tk.Frame(win, bg=BG)
        custom_row.pack(fill="x", padx=36, pady=(0, 6))
        tk.Label(custom_row, text="Custom level count:", bg=BG, fg=FG,
                 font=("Segoe UI", 9)).pack(side="left")
        customvar = tk.StringVar(value="4")
        tk.Entry(custom_row, textvariable=customvar, width=5,
        ...
        def ok():
            ...
            if mv == "1":
                ...
                try:
                    lv = max(1, int(customvar.get()))
                except Exception:
                    lv = 1
            choice["v"] = (fmtvar.get(), lv)
            win.destroy()

        tk.Button(win, text="  Add Texture  ", command=ok,
                  bg=BTN, fg=FG, activebackground=ACCENT,
                  relief="flat", padx=20, pady=6,
                  font=("Segoe UI", 10)).pack(pady=14)

        # Force geometry calculation BEFORE grab_set so the window draws correctly
        win.update_idletasks()
        # Centre over parent
        pw = self.winfo_rootx(); ph = self.winfo_rooty()
        pw2 = self.winfo_width(); ph2 = self.winfo_height()
    ...
    def _rename_sel(self):
        ...
        win=tk.Toplevel(self); win.title("Rename Texture"); win.configure(bg=BG); win.grab_set()
        tk.Label(win,text="New name (no extension):",bg=BG,fg=FG,
                 font=("Segoe UI",10),padx=16,pady=8).pack()
        var=tk.StringVar(value=Path(cur).stem)
        ent=tk.Entry(win,textvariable=var,bg=ENTRY,fg=FG,insertbackground=FG,
                     font=("Consolas",11),width=32); ent.pack(padx=16,pady=4); ent.focus_set()
        def ok():
            nm=var.get().strip()
            if nm:
            ...
            win.destroy()
        tk.Button(win,text="Rename",command=ok,bg=BTN,fg=FG,activebackground=ACCENT,
                  relief="flat",padx=16,pady=5).pack(pady=10)
    ...
    def _show_formats(self):
        win=tk.Toplevel(self); win.title("Supported Formats"); win.configure(bg=BG); win.grab_set()
        text=("Platform detection (from magic bytes):\n"
              "  PC      .ytd        RSC7  Little-Endian  zlib\n"
              "  PS3     .ctd/.xtd   7CSR  Big-Endian     zlib\n"
    ...
    def _about(self):
        win=tk.Toplevel(self); win.title("About"); win.configure(bg=BG); win.grab_set()
        text=("RAGE Console Texture Editor - Python Port\n\n"
              ...
              "PC and PS3 paths use stdlib zlib and run anywhere.")
        tk.Label(win,text=text,bg=BG,fg=FG,font=("Consolas",9),justify="left",
                 padx=20,pady=12).pack()
        tk.Button(win,text="Close",command=win.destroy,bg=BTN,fg=FG,relief="flat",
                  padx=20,pady=4).pack(pady=(0,12))
