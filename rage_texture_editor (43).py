...
class XtdWriter:
    ...
    def build(self, compress=True):
        ...
        for idx in order:
            ...
            # dword_12 encodes mip count. The reader takes the LITTLE-ENDIAN
            # dword L at d3d+0x2C and computes:
            #   MaxMip = EndianChangeDWORD((L&0xC0000000)>>6 | (L&0x00030000)<<10)
            # Build L so MaxMip == levels-1 (verified for 0..15 mips), then store
            # L as a little-endian dword (patch_u32_be expects a value it will
            # byte-swap, so pass EndianChangeDWORD(L) to land L's bytes in LE).
            maxmip = max(0, levels - 1)
            X = EndianChangeDWORD(maxmip)
            L = (((X & 0x03000000) << 6) & 0xC0000000) | \
                (((X & 0x0C000000) >> 10) & 0x00030000)
            sysseg.patch_u32_be(d3d+0x2C, EndianChangeDWORD(L))  # dword_12 (LE on disk)
            # dword_13: stock files store 0x00000200 here (BE) for single-mip
            # textures; the mip-chain address only differs when levels>1.
            if levels > 1:
                sysseg.patch_u32_be(d3d+0x30, (gpu_paged & 0xFFFF0000) | 0x0A00)
            else:
...
class TextureDict:
    ...
    def export_dds_mip(self, tex, mip_level=0):
        ...
        if tex.platform in ("Xbox 360", "GTA4-Xbox 360"):
            ...
            for i in range(mip_level):
                s = _xbox_format_setup(gpu_fmt, dims[i][0], dims[i][1])
                skip += (s[0] if s else 0)
            data = untile_and_deswap(self._dec[off+skip:], gpu_fmt, mw, mh, tex.endian)
            return make_dds(gpu_fmt, mw, mh, 1, data)
        elif tex.platform == "PS3":
            off = tex.tex_offset + self.cpu_size
            skip = 0
            for i in range(mip_level):
                skip += ps3_data_size(gpu_fmt, dims[i][0], dims[i][1])
            size = ps3_data_size(gpu_fmt, mw, mh)
            data = self._dec[off+skip:off+skip+size]
            return make_dds(gpu_fmt, mw, mh, 1, data)
            ...
            return make_dds(tex.gpu_fmt or 'GPUTEXTUREFORMAT_DXT4_5', mw, mh, 1, data)
    ...
    def save(self, out_path):
        ...
        if dict_dirty:
            compress = (self.platform != "Xbox 360") or \
    ...
    def _convert_ps3_inplace(self, tex, new_gpu, rgba, w, h):
        ...
        # lay them out with the page allocator (PS3 base unit 0x1000).
        per_tex = []
        for t in self.textures:
            if t is tex:
                fmt = new_gpu; trgba, tw, th = rgba, w, h
            else:
                fmt = t.gpu_fmt; trgba, tw, th = self._decoded_rgba(t)
            linear = encode_for_format(fmt, trgba, tw, th)
        ...
        sizes = [p[3] for p in per_tex]
        offsets, total, _flag = rage_page_layout(sizes, 0x1000)
        new_gpu_block = bytearray(total)
    ...
    def save_rebuilt(self, out_path, compress=True):
        ...
        add/remove) and write it. Routes to the correct platform writer.
        """
    ...
    def to_texture_list(self):
        ...
        for t in self.textures:
            ...
            if nlev > 1:
                # build a full chain by downsampling the base image
                img = Image.frombytes("RGBA", (w, h), rgba)
                chain = bytearray()
                for i in range(nlev):
                    mw = max(1, w >> i); mh = max(1, h >> i)
                    lvl_img = img if i == 0 else img.resize((mw, mh), Image.LANCZOS)
                    chain += encode_for_format(gpu, lvl_img.tobytes(), mw, mh)
                data = bytes(chain)
            else:
                ...
                try:
                    ...
                    for e in arc.iter_files():
                        if e.name.lower().endswith(".rpf"):
                            try:
                                nraw = arc.extract_to_disk_bytes(e)
...
class App(tk.Tk):
    ...
    def _build_mip_selector(self, tex):
        """Show a row of mip-level buttons for multi-mip textures; hide for single."""
        ...
        for lvl,(mw,mh) in enumerate(dims):
            ...
            b=tk.Button(self._mipbar, text=f"{lvl}: {mw}×{mh}",
                        command=lambda l=lvl: self._select_mip(tex, l),
                        bg=(ACCENT if sel else BTN), fg=(BG if sel else FG),
